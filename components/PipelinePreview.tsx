...
function PeptideEngine() {
  ...
  // rAF-throttled mouse tracking for 3D rotation
  ...
  const sectors = [
    { name: "Endometriosis", color: "#3A2E45", label: "Non-Hormonal", status: "IND Cleared" },
  ...
  return (
    <div
        ref={(el) => { (containerRef as React.MutableRefObject<HTMLDivElement | null>).current = el; setVisRef(el); }}
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
         <div className="relative w-72 h-72 flex items-center justify-center">
            ...
            <motion.div
               className="absolute inset-0 border border-gray-200 rounded-full opacity-30 will-change-transform"
               animate={isVisible ? { rotate: 360 } : undefined}
               transition={isVisible ? { duration: 40, repeat: Infinity, ease: "linear" } : { duration: 0 }}
            />
            <motion.div
               className="absolute inset-4 border border-gray-300/50 rounded-full border-dashed will-change-transform"
               animate={isVisible ? { rotate: -360 } : undefined}
               transition={isVisible ? { duration: 50, repeat: Infinity, ease: "linear" } : { duration: 0 }}
            />
      ...
      {sectors.map((sector, i) => {
         ...
         return (
            <motion.div
               key={i}
               className="absolute transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-20 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-primary focus-visible:ring-offset-2 rounded-full"
               style={{ left: `${x}%`, top: `${y}%` }}
               animate={{ scale: isActive ? 1.1 : 0.9, opacity: isActive ? 1 : 0.4 }}
               role="button"
               tabIndex={0}
               aria-label={`Select ${sector.label} sector`}
               onClick={() => setActiveSector(i)}
               onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setActiveSector(i); } }}
            >
               <div 
      ...
      <div className="absolute bottom-8 left-8 right-8 flex justify-between items-end pointer-events-none">
         <div className="text-[9px] font-mono text-gray-500/90 space-y-1">
            <div className="flex items-center gap-2">
               <div className={`w-1.5 h-1.5 rounded-full bg-green-500 ${isVisible ? 'animate-pulse' : ''}`} />
