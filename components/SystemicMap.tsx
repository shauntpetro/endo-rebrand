...
export default function SystemicMap({ theme = "light" }: { theme?: "light" | "dark" }) {
  ...
  // Pause beams while inspecting; freeze all looping motion when the OS prefers reduced motion.
  const beamsActive = isVisible && activeNode === null && !reduced;
  ...
  return (
    <div ref={visRef as React.RefObject<HTMLDivElement>} role="group" aria-label="Systemic impact map: interactive diagram of body sites affected by endometriosis" className={`w-full h-full relative ${isDark ? 'bg-plum-dark/50 border-plum-primary/30' : 'bg-bone border-plum-dark/10'} overflow-hidden rounded-lg border group transition-colors duration-500`}>

      {/* Persistent live region — announces the active site reliably (content in a
          conditionally-mounted aria-live element is often missed by screen readers) */}
      <div aria-live="polite" className="sr-only">
        {activeNodeData ? `${activeNodeData.label}: ${activeNodeData.description}` : ""}
      </div>
      ...
      <AnimatePresence>
        {activeNodeData && (
          <motion.div
            ...
            className={`absolute top-4 left-4 z-40 ${isDark ? 'bg-plum-dark/95 border-plum-primary/50' : 'bg-bone-raised/95 border-plum-dark/10'} backdrop-blur-xl border px-5 py-4 rounded-lg shadow-lg max-w-[240px]`}
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              />
              <p className={`text-xs font-bold ${isDark ? 'text-gold-primary' : 'text-plum-primary'} uppercase tracking-wider`}>
                {activeNodeData.label}
              </p>
            </div>
            <p className={`text-[11px] ${isDark ? 'text-gray-300' : 'text-black-soft'} leading-relaxed`}>
        ...
        {nodes.map((node, i) => (
            i > 0 && (
                <g key={i}>
                    ...
                    {/* Particle Flow Animation — hidden at rest so it fully freezes when paused/reduced */}
                    <motion.circle
                        r="2"
                        fill="#C9A961"
                        cx={`${nodes[0].x}%`}
       ...
       <div className="relative h-[95%] w-full max-w-[400px]">
        {nodes.map((node, i) => (
            <motion.div
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setPinnedNode((p) => (p === node.id ? null : node.id)); }
                  else if (e.key === "Escape") { setPinnedNode(null); setHoverNode(null); }
                }}
            >
                {/* Hairline ring — flat halo replacement, always visible in static state */}
                <div
                    ...
                    style={{
                        ...
                        left: -node.r * 0.5,
                        top: -node.r * 0.5,
                        borderColor: `${node.color}${node.id === 0 ? "59" : "33"}`,
                        opacity: isVisible ? 1 : 0,
                        transition: "opacity 0.6s ease",
                    }}
                />

                {/* Origin "emitter" pulse — freezes when reduced motion is preferred */}
                {node.id === 0 && (
                    <motion.div
                        className="absolute rounded-full border pointer-events-none"
                        style={{ width: node.r * 2, height: node.r * 2, borderColor: "#C9A96166" }}
                        animate={isVisible && !reduced ? { scale: [1, 2.6], opacity: [0.6, 0] } : undefined}
                        transition={isVisible && !reduced ? { duration: 2.6, repeat: Infinity, ease: "easeOut" } : { duration: 0 }}
                    />
                )}

                {/* Crisp core — flat fill with a hairline border, no glow */}
                <div
                    className={`relative rounded-full border transition-all duration-500 ease-out ${activeNode === node.id ? "scale-125" : ""}`}
                    style={{
                        width: node.r * 2,
                        height: node.r * 2,
                        backgroundColor: activeNode === node.id ? "#FAF6EC" : node.color,
                        borderColor: activeNode === node.id ? "#4A3F5C" : (isDark ? "rgba(255,255,255,0.85)" : "rgba(46,38,58,0.15)"),
                    }}
                >
                    {activeNode === node.id && (
      ...
      {/* Legend Panel - Bottom Right — transform-only reveal, content visible in static state */}
      <div
        className={`reveal-rise absolute bottom-4 right-4 z-30 ${isDark ? 'bg-plum-dark/90 border-plum-primary/30' : 'bg-bone-raised/90 border-plum-dark/10'} backdrop-blur-md border rounded-lg shadow-lg px-5 py-4 max-w-[240px]`}
        style={{ animationDelay: "0.4s" }}>
        <h3 className={`text-[10px] font-bold ${isDark ? 'text-gold-primary' : 'text-gold-deep'} uppercase mb-2 tracking-[0.1em]`}>
          Systemic Impact Map
        </h3>
        <p className={`text-[11px] ${isDark ? 'text-gray-300' : 'text-black-soft'} leading-relaxed mb-3`}>
          Endometriosis is systemic. Hover or click a site to inspect; beams pause while you read.
        </p>
        <div className="flex items-center gap-4 mb-3">
          <div className="flex items-center gap-2">
            <div aria-hidden="true" className={`w-3 h-3 rounded-full ${isDark ? 'bg-gold-primary' : 'bg-plum-primary'}`} />
            <span className={`text-[10px] font-semibold ${isDark ? 'text-gray-300' : 'text-black-soft'} uppercase tracking-[0.05em]`}>Origin</span>
          </div>
          <div className="flex items-center gap-2">
            <div aria-hidden="true" className="w-3 h-3 rounded-full bg-gold-primary" />
            <span className={`text-[10px] font-semibold ${isDark ? 'text-gray-300' : 'text-black-soft'} uppercase tracking-[0.05em]`}>Target</span>
          </div>
        </div>
        <div className={`pt-3 border-t ${isDark ? 'border-plum-primary/30' : 'border-plum-dark/10'}`}>
          <p className={`text-[9px] font-bold uppercase tracking-[0.08em] ${isDark ? 'text-gold-primary' : 'text-gold-deep'} mb-1.5`}>Associated comorbidities</p>
          <ul className={`text-[10px] ${isDark ? 'text-gray-300' : 'text-black-soft'} space-y-1`}>
