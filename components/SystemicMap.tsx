...
export default function SystemicMap({ theme = "light" }: { theme?: "light" | "dark" }) {
  ...
  return (
    <div className={`w-full h-full relative ${isDark ? 'bg-plum-dark/50 border-plum-primary/30' : 'bg-white border-gray-200'} overflow-hidden rounded-sm border shadow-inner group transition-colors duration-500`}>
      ...
      <AnimatePresence>
        {activeNodeData && (
          <motion.div
            ...
            className={`absolute top-4 left-4 z-40 ${isDark ? 'bg-plum-dark/95 border-plum-primary/50' : 'bg-white/95 border-gray-200'} backdrop-blur-xl border px-5 py-4 rounded-sm shadow-xl max-w-[240px]`}
          >
      ...
      <motion.div 
        ...
        animate={{ top: ["5%", "95%"], opacity: [0, 0.4, 0] }}
      ...
      <div className="absolute inset-0 w-full h-full z-20">
        {nodes.map((node, i) => (
            ...
            >
                ...
                {/* Core Node */}
                <div 
                    className={`relative rounded-full border border-white shadow-md transition-all duration-300 ${activeNode === node.id ? 'scale-125 ring-4 ring-plum-primary/20' : ''}`}
                    style={{ 
                        width: node.r * 2, 
                        height: node.r * 2,
      ...
      <div className={`absolute bottom-4 right-4 z-30 ${isDark ? 'bg-plum-dark/90 border-plum-primary/30' : 'bg-white/90 border-gray-200'} backdrop-blur-md border rounded-sm shadow-lg px-5 py-4 max-w-[220px]`}>
